...
import { addSubscription, assertSourceContributor, deleteSource, setSourceEnabled, setSourceFollowing, setSourceSyncInterval, syncSource } from "../../../lib/store";

export async function POST(request: Request) {
  try {
    assertSameOrigin(request);
    const user = await requireSessionUser(env, request);
    const body = await request.json() as { action: "add" | "follow" | "toggle" | "sync" | "delete" | "set-interval"; name?: string; url?: string; category?: string; id?: number; enabled?: boolean; following?: boolean; minutes?: number | null };
    ...
    if (body.action === "set-interval") { await setSourceSyncInterval(env, id, body.minutes == null ? null : Number(body.minutes)); return Response.json({ ok: true }); }
